...
const RestockStorage = ({ onClose, user }) => {
  ...
  const loadActiveBatches = async () => {
    try {
      const data = await fetchActiveBatches(user.business_id);
      setStorageData(data);
    } catch (error) {
  ...
  const handleConsumeRestock = async (restock) => {
    const productCategoryId =
      restock.productcategory?.productcategoryid;
    const currentStock =
      restock.productcategory?.currentstock ?? 0;
                  ...
                  <tbody>
                    {storageData.map((restock) => {
                      const current =
                        restock.productcategory?.currentstock ?? 0;
                      const reorder =
                        restock.productcategory?.reorderpoint ?? 0;
                      ...
                      return (
                        <tr key={restock.restockid}>
                          ...
                          <td>
                            {restock.datereceived
                              ? new Date(restock.datereceived).toLocaleDateString()
                              : "-"}
                          </td>
                          <td>
